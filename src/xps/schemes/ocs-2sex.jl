...
"""
    hhocs(test, foo, bar, lmp, ngn, trait, fixed, plan, dF, F0; ε = 1e-6)
Optimal contribution selection with `H`(homozygous) relationship matrix for EBV,
`H` relationship matrix for constraint on `foo`.xy and `foo`.ped in directory
`test` for `ngn` generations. SNP linkage information are in DataFrame `lmp`.
The results are saved in `bar`.xy, `bar`.ped in directory `test`. The selection
is on a single trait `trait` with fixed effects `fixed`, which is a column name
vector in pedigree DataFrame. Parents are sampled according to `plan`. The
constraint ΔF is `dF`. `F0` is the inbreeding coefficient of the `foo`
population.
...
"""
    hgocs(test, foo, bar, lmp, ngn, trait, fixed, plan, dF, F0; ε = 1e-6)
Optimal contribution selection with `H`(homozygous) relationship matrix for EBV,
`G` relationship matrix for constraint on `foo`.xy and `foo`.ped in directory
`test` for `ngn` generations. SNP linkage information are in DataFrame `lmp`.
The results are saved in `bar`.xy, `bar`.ped in directory `test`. The selection
is on a single trait `trait` with fixed effects `fixed`, which is a column name
vector in pedigree DataFrame. Parents are sampled according to `plan`. The
...
[`ggocs`](@ref), [`agocs`](@ref).
"""
function hgocs(test, foo, bar, lmp, ngn, trait, fixed, plan, dF, F0; ε = 1e-6)
